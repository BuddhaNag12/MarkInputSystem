...
<v-container>
 <div class="text-center">
          ...
          <v-list-item>
           ...
           <v-list-item>
            <v-btn  text  :to="'/update_profile/'+user.data.id">
               <v-icon dark class="pr-2">mdi-wrench</v-icon>
              UPDATE PROFILE</v-btn>
          </v-list-item>
          <v-list-item>
             <v-dialog v-model="dialog" persistent max-width="600px">
      <template v-slot:activator="{ on }">
        <v-btn text  v-on="on"><v-icon class="pr-2">mdi-account-edit</v-icon> add Subject</v-btn>
      </template>
      <v-card>
        <v-card-title>
          ...
          <v-container>
            <v-layout row class="justify-center">
              ...
              <form @submit.prevent="addSubject" >
               ...
               <v-select :items="['1st semester','2nd semester','3rd semester','4th semester','5th semester','6th semester']" v-model="sem" label="Semester"></v-select>
                <v-select :items="['pass','honors']" v-model="type" label="type" @change="changeInput"></v-select>
  ...
  <v-list-item>
           ...
           <v-list-item>
            <v-btn to="/ut1_marks" text><v-icon class="pr-2">mdi-counter</v-icon> Unit Test I Marks</v-btn>
          </v-list-item>
                <v-list-item>
    ...
    <v-card  color="blue lighten-2 "  >
      <v-card-text > 
          <p class="display-1 white--text font-italic text-uppercase pb-2 " >Welcome to marks input system</p>
      </v-card-text>
